...
namespace LoginSystem
{
    public class SignUpManager : MonoBehaviour
    {
        ...
        public void OnCreateAccountButtonPressed()
        {
            ...
            // Name validation (simple, adjust as needed)
            if (name.Length < 2) // Example: Minimum name length
            {
                panelsManager.ShowError("Name must be at least 2 characters long.");
                return;
            }
            ...
            {
                panelsManager.ShowError("Password must be at least 8 characters long, contain uppercase, lowercase, and a number.");
                return;
            }
            
            if (password != confirmPassword) // NEW
            {
                panelsManager.ShowError("Passwords do not match.");
                return;
            }
        ...
        {
            // Example: Minimum 8 characters, at least one uppercase, one lowercase, one digit.
            // Adjust complexity as per your game's security requirements.
            if (password.Length < 8)
            {
                // We could store the specific reason for weak password in a temp variable
                // and pass it to ShowError for more granular feedback.
                return false;
            }
            ...
            // if (!System.Text.RegularExpressions.Regex.IsMatch(password, "[!@#$%^&*()_+=\[{\]};:<>|./?,-]")) return false; // Uncomment for special character requirement
            return true;
        }
